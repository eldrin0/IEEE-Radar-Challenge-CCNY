...
saveFolder = 'C:\Users\leeju\Documents\HeatMap Data'; % <-- Custom Path Set Here
saveDataToDisk = true; % Set to false to disable saving entirely
% ------------------------------

% Create the directory if it doesn't exist
if saveDataToDisk && ~exist(saveFolder, 'dir')
    mkdir(saveFolder);
    fprintf('Created directory for saving raw data: %s\n', saveFolder);
end
...
while toc(tTotal) < tCapture
    ...
    if isFirstSweep
        ...
        % --- Save ONE Background Data File ---
        if saveDataToDisk
            bgFilename = fullfile(saveFolder, 'Background_Data.mat');
            save(bgFilename, 'backgroundMask_dB', 'validRanges', 'azAngles');
            fprintf('Saved background data to: %s\n', bgFilename);
        end
        isFirstSweep = false;
    end
    ...
    % --- Save ONLY Raw Heatmap Data to Disk ---
    if saveDataToDisk
        dataFilename = fullfile(saveFolder, sprintf('Raw_Heatmap_Data_%04d.mat', sweepCount));
        save(dataFilename, 'heatmapMatrix', 'heatmap_dB');
    end
    sweepCount = sweepCount + 1;
    
    time_total_sweep = toc(sweepStart);
    fprintf('   %5.3f s   |     %5.3f s     |   %5.3f s\n', ...
        time_total_sweep, time_collection, time_plot);
end
