...
for i = 1:numFiles
    ...
    % 3. CFAR Scope
    [detMap, noiseMap] = apply_cfar(magnitudeMTI, 10, 2, 20, 10, 5e-3);
    
    detMapPlot = detMap;
    detMapPlot(detMapPlot <= 0) = 1;
    scopeCFAR(detMapPlot, rGridMTI(maskMTI), sGridMTI);

    targets = extract_targets(detMap, noiseMap, 12);
    if ~isempty(targets)
...
%   Outputs a matrix where each row is a target: [Range Index, Doppler Index, Peak Value]
    ...
    for t = 1:max_targets
        ...
        % r_idx is the Row (Vertical boundary)
        % v_idx is the Column (Horizontal boundary)
        r1 = max(1, r_idx - mask_w);                  % Top edge
        r2 = min(size(temp_map,1), r_idx + mask_w);   % Bottom edge
        v1 = max(1, v_idx - mask_w);                  % Left edge
        v2 = min(size(temp_map,2), v_idx + mask_w);   % Right edge
